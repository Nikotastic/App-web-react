...
const Reportes = () => {
  ...
  const movimientosFiltrados = movimientos.filter((mov) => {
    ...
    return fecha.getMonth() === parseInt(mesSeleccionado) && fecha.getFullYear() === parseInt(anioSeleccionado);
  });
    ...
    <div className="layout flex h-[100vh]">
      ...
      <main className="main-content flex flex-1 flex-col items-center pt-8 px-6 overflow-auto text-white">
        ...
        <div className="mb-6 flex gap-4 items-center">
          ...
          >
            {[...Array(12).keys()].map((m) => (
              <option key={m} value={m}>{new Date(0, m).toLocaleString("es-ES", { month: "long" })}</option>
            ))}
          </select>
          <select
            className="bg-gray-800 text-white rounded p-2"
            ...
            <tbody>
              {topGastos.map((gasto) => (
                <tr key={gasto.id} className="border-t border-gray-700">
                  <td className="py-2">{new Date(gasto.fecha.seconds * 1000).toLocaleDateString()}</td>
